...
委托构造函数：使用它**所属类**的**其他构造函数**执行它的初始化过程。
...
**注意**：最好让构造函数初始值的顺序和成员声明的顺序保持一致。尽量避免用某些成员初始化其他成员。
...
尽管this是隐式的，但仍然需要遵守初始化规则，意味着（在默认情况下）我们不能将this绑定到一个常量对象上（指针指向的对象必须与指针的类型相同。可以用非const来初始化cosnt指针，但反过来不行，此处的情况就是反过来，用const来初始化非const指针）。因此，不能在一个常量对象上调用普通的成员函数。

常函数，加了const之后，this变成 const AA * const this，此时this可以绑定常量对象
