...
成员函数通过一个this的隐式参数来访问调用它的那个对象。当我们调用一个成员函数时，用请求该函数的对象的地址初始化this。即
...
默认情况下，this的类型是指向类类型的非常量版本的常量指针。 AA * const this

尽管this是隐式的，但仍然需要遵守初始化规则，意味着（在默认情况下）我们不能将this绑定到一个常量对象上。因此，不能在一个常量对象上调用普通的成员函数。
