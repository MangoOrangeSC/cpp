...
- **函数声明**：函数的名字也必须在使用前声明。函数的声明和定义唯一的区别是声明无需函数体，用一个分号替代。
...
- 管理数组形参有三种方法：1数组本身包含结束标记；2传递指向数组首尾元素的指针；3专门定义表示数组大小的形参。
- 允许将变量定义成数组的引用
...
- 指针的类型与所指向的对象类型必须严格匹配（均为同一类型int、double等），除了两种特殊情况，一种为const
- 建议：初始化所有指针。
- 理解复合类型的声明
	- 从右向左阅读复合类型的定义 
